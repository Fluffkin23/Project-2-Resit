    ...
    while ($row = $result->fetch_assoc()) {
        $product = <<<DELIMETER
            <tr>
                ...
                <td>{$row['status']} </td>
                <td> <a class='btn btn-danger' href = "status_InProgess.php?update&id={$row['id']}"><span class='glyphicon glyphicon-check'></span></a></td>
               <td> <a class='btn btn-danger' href = "status_Done.php?update&id={$row['id']}"><span class='glyphicon glyphicon-remove-circle'></span></a></td>
            </tr>
            DELIMETER;
        echo $product;
    }
...
function request_done()
{
    $connection = mysqli_connect("localhost", "root", "", "service_it") or die("Connection Failed" . mysqli_connect_error());
    $status = "DONE";
    $sql = "SELECT * FROM reqeust_services WHERE status=?";
    $stmt = $connection->prepare($sql);
